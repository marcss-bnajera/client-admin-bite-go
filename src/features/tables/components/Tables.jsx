    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                {loading ? (
                    <p className="text-[#6B6B6B] text-sm col-span-3 text-center py-10">Cargando mesas...</p>
                ) : paginated.length === 0 ? (
                    <p className="text-[#6B6B6B] text-sm col-span-3 text-center py-10">No hay mesas registradas</p>
                ) : paginated.map((mesa) => (
                    <div key={mesa._id} className="bg-white rounded-2xl border border-[#E8D8C3] shadow-sm p-5 hover:shadow-md transition-shadow">
                        ...
                        <div className="flex justify-end gap-2 border-t border-[#E8D8C3] pt-3">
                            <button onClick={() => handleEdit(mesa)} className="p-2 rounded-lg hover:bg-[#F2E6D9] text-[#E67E22] transition-colors" title="Editar">
                                <Pencil size={15} />
                            </button>
                            <button onClick={() => handleDelete(mesa)} className="p-2 rounded-lg hover:bg-red-50 text-[#C0392B] transition-colors" title="Eliminar">
                                <Trash2 size={15} />
                            </button>
                        </div>
                    </div>
